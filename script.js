// Book class
class Book {
  constructor(title, author, numpages, readStatus, cover) {
    this.title = title;
    this.author = author;
...
const myLibrary = [
  {
    ...
    cover:
      "http://books.google.com/books/content?id=7ctWAAAAQBAJ&printsec=frontcover&img=1&zoom=5&edge=curl&source=gbs_api",
    index: 0,
    numpages: 128,
  ...
  },
  {
    ...
    readStatus: "Reading",
    cover:
      "http://books.google.com/books/content?id=tKupDwAAQBAJ&printsec=frontcover&img=1&zoom=5&edge=curl&source=gbs_api",
    index: 1,
  },
  {
    ...
    readStatus: "Read",
    cover:
      "http://books.google.com/books/content?id=v5fFDwAAQBAJ&printsec=frontcover&img=1&zoom=5&edge=curl&source=gbs_api",
    index: 2,
  },
];
...
function render() {
  ...
  bookShelfContainer.appendChild(createContainer); // Append book shelf div to body
  ...
  myLibrary.forEach((book) => {
    ...
    const indexNumber = myLibrary.indexOf(book); // Index of book in current myLibrary array
...
function addBook(event) {
  ...
  const generateBookInfo = (book) => {
    ...
    const { title } = book[`ISBN:${apiData.isbn}`];
...
function removeBook(e) {
  const bookIndex = Number(
    e.target.parentElement.parentElement.attributes[0].nodeValue
  );
